...
import { Coffee, ListTodo, CheckSquare, Target, Rocket, PieChart, LayoutDashboard } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({ currentView, onChangeView, orientation = 'horizontal', isDarkMode = true }) => {
  const tabs = [
    { id: AppView.FOCUS, label: 'Focus', icon: Coffee },
    { id: AppView.TODOS, label: 'To-Dos', icon: ListTodo },
  ...
  // Horizontal (Mobile)
  return (
    <div className={`backdrop-blur-xl p-1.5 rounded-2xl flex justify-between items-center mb-6 border shadow-xl sticky top-0 z-30 ${
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            key={tab.id}
            onClick={() => onChangeView(tab.id)}
            className={`flex flex-col items-center justify-center py-2.5 px-2 flex-1 rounded-xl transition-all duration-300 relative overflow-hidden ${
              isActive
